...
const EndometrialPreparationCard: React.FC<EndometrialPreparationCardProps> = ({
  patientId,
}) => {
  const [endometrialPreparationData, setEndometrialPreparationData] =
    useState<EndometrialPreparationData>(() => ({
      prescription: {
  ...
  const [
    isEndometrialPreparationDialogOpen,
    setIsEndometrialPreparationDialogOpen,
  ] = useState(false);
  const [isLoadingEndometrialPreparation, setIsLoadingEndometrialPreparation] =
    useState(false);
  const [errorEndometrialPreparation, setErrorEndometrialPreparation] =
    useState<string | null>(null);

  const [isSavingEndometrialPreparation, setIsSavingEndometrialPreparation] =
    useState(false);
  const [saveErrorEndometrialPreparation, setSaveErrorEndometrialPreparation] =
    useState<string | null>(null);
  const [
    saveSuccessEndometrialPreparation,
    setSaveSuccessEndometrialPreparation,
  ] = useState(false);
  ...
  const [errorMedicationList, setErrorMedicationList] = useState<string | null>(
    null
  );

  const [currentPrescriptionItem, setCurrentPrescriptionItem] =
  ...
  const [isPrescriptionItemDialogOpen, setIsPrescriptionItemDialogOpen] =
    useState(false);
  ...
  useEffect(() => {
    const fetchEndometrialPreparationData = async () => {
      ...
      try {
        ...
        const data = (await getEndometrialPreparationProcess(
          patientId
        )) as EndometrialPreparationData;

        console.log("Fetched endometrial preparation data:", data); // For debugging
        if (data) {
          const validStatuses: EndometrialPreparationStatus[] = [
            "IN_PROGRESS",
            "COMPLETED",
            "CANCELLED",
          ];
          const validatedStatus: EndometrialPreparationStatus =
            validStatuses.includes(data.status) ? data.status : "IN_PROGRESS";

          const itemsWithDrugNames = data.prescription.items.map((item) => ({
            ...item,
            drugName:
              medicationList.find((med) => med.id === item.medicationId)
        ...
        console.error(
          "Failed to fetch endometrial preparation process:",
          error
        );
        setErrorEndometrialPreparation(
          "Không thể tải dữ liệu chuẩn bị nội mạc tử cung."
        );
        setEndometrialPreparationData(() => ({
          prescription: {
            dateIssued: new Date().toISOString().split("T")[0],
  ...
  const handleEndometrialPreparationChange = (
    field: string,
    value,
    subField?: string
  ) => {
  ...
  const handleDeleteItem = (index: number) => {
    setEndometrialPreparationData((prev) => {
      const updatedItems = prev.prescription.items.filter(
        (_, i) => i !== index
      );
      return {
        ...prev,
  ...
  const handleSaveEndometrialPreparation = useCallback(async () => {
    if (!patientId) {
      setSaveErrorEndometrialPreparation(
        "Dữ liệu bệnh nhân không hợp lệ để lưu."
      );
      return;
    }
    ...
    try {
      const dataToSend = {
        ...
        prescription: {
          ...
          items: endometrialPreparationData.prescription.items.map(
            ({ drugName, ...rest }) => rest
          ),
        },
      };
      ...
      setSaveErrorEndometrialPreparation(
        error.response?.data?.message || "Đã xảy ra lỗi khi lưu."
      );
    } finally {
  ...
  const handleCompleteEndometrialPreparation = useCallback(async () => {
    if (!patientId) {
      setErrorEndometrialPreparation(
        "Dữ liệu bệnh nhân không hợp lệ để hoàn thành."
      );
      return;
    }
    ...
    try {
      ...
      console.error(
        "Failed to complete endometrial preparation process:",
        error
      );
      setErrorEndometrialPreparation(
        error.response?.data?.message ||
          "Không thể hoàn thành quá trình chuẩn bị nội mạc tử cung."
      );
    } finally {
  ...
  const handleCancelEndometrialPreparation = useCallback(async () => {
    ...
    try {
      ...
      setErrorEndometrialPreparation(
        error.response?.data?.message ||
          "Không thể hủy quá trình chuẩn bị nội mạc tử cung."
      );
    } finally {
    ...
    <Card className="p-6 bg-white border border-[color:var(--card-border)]">
      <h3 className="text-lg font-semibold mb-4 flex items-center justify-between text-[color:var(--text-accent)]">
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={() => setIsEndometrialPreparationDialogOpen(true)}
          className="ml-2"
          disabled={isOperationInProgress}
        >
          <Edit className="w-4 h-4 mr-2" />{" "}
          {endometrialPreparationData.prescription.items.length > 0 ||
          endometrialPreparationData.startDate
            ? "Chỉnh sửa"
            : "Nhập liệu"}
        </Button>
      </h3>
      ...
      {!isLoadingEndometrialPreparation && !errorEndometrialPreparation && (
        <>
          {!endometrialPreparationData.prescription.items.length &&
          !endometrialPreparationData.startDate &&
          !endometrialPreparationData.drugResponse &&
          endometrialPreparationData.status === "IN_PROGRESS" ? (
            <p className="text-center text-[color:var(--text-secondary)] mb-4">
              Chưa có dữ liệu chuẩn bị nội mạc tử cung.
            </p>
          ) : (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">
                    {renderValue(
                      formatDate(endometrialPreparationData.startDate)
                    )}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {renderStatusInVietnamese(
                      endometrialPreparationData.status
                    )}
                  </p>
                </div>
              </div>
                ...
                <div className="mt-4 border-t pt-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <p className="font-medium">
                        {renderValue(
                          formatDate(
                            endometrialPreparationData.prescription.dateIssued
                          )
                        )}
                      </p>
                    </div>
                    <div className="col-span-1 md:col-span-2">
                      ...
                      <p className="font-medium">
                        {renderValue(
                          endometrialPreparationData.prescription.notes
                        )}
                      </p>
                    </div>
                  </div>
                  ...
                  {endometrialPreparationData.prescription.items.length > 0 ? (
                    <div className="space-y-3 max-h-[200px] overflow-y-auto pr-2 custom-scrollbar">
                      ...
                      {endometrialPreparationData.prescription.items.map(
                        (item, index) => (
                          <Card
                            key={index}
                            className="p-4 border-l-4 border-[color:var(--deep-taupe)] bg-gray-50 flex justify-between items-center"
                          >
                            <div>
                              <p className="font-semibold text-[color:var(--text-accent)]">
                                {renderValue(item.drugName)}
                              </p>
                              <p className="text-sm text-[color:var(--text-secondary)]">
                                Liều: {renderValue(item.dosage)} | Tần suất:{" "}
                                {renderValue(item.frequency)} | Thời lượng:{" "}
                                {renderValue(item.duration)} | SL:{" "}
                    ...
                    <p className="text-sm text-[color:var(--text-secondary)]">
                      Chưa có mục thuốc nào trong đơn.
                    </p>
                  )}
                </div>
              )}
      ...
      <Dialog
        open={isEndometrialPreparationDialogOpen}
        onOpenChange={setIsEndometrialPreparationDialogOpen}
      >
        <DialogContent className="sm:max-w-[700px] p-6 max-h-[90vh] overflow-y-auto">
          ...
          <DialogHeader>
            <DialogTitle>
              {isEditable ? "Chỉnh sửa" : "Xem"} Chuẩn bị nội mạc tử cung
            </DialogTitle>
            <DialogDescription>
              {isEditable
                ? "Cập nhật thông tin và đơn thuốc cho quá trình chuẩn bị nội mạc tử cung."
                : "Quá trình đã hoàn thành hoặc bị hủy, không thể chỉnh sửa."}
            </DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) =>
                  handleEndometrialPreparationChange(
                    "startDate",
                    e.target.value
                  )
                }
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  handleEndometrialPreparationChange("drugResponse", value)
                }
                disabled={!isEditable || isSavingEndometrialPreparation}
              >
                <SelectTrigger>
            ...
            <div className="col-span-1 md:col-span-2 border-t pt-4 mt-4">
              ...
              <div className="mt-4">
                ...
                {endometrialPreparationData.prescription.items.length === 0 ? (
                  <p className="text-sm text-[color:var(--text-secondary)]">
                    Chưa có mục thuốc nào được thêm.
                  </p>
                ) : (
                        ...
                        >
                          ...
                          <div className="flex space-x-2">
                            <Button
                              ...
                              disabled={
                                !isEditable || isSavingEndometrialPreparation
                              }
                            >
          ...
          <DialogFooter className="sticky bottom-[-18px] bg-white z-10 border-t pt-4">
            ...
            {saveErrorEndometrialPreparation && (
              <p className="text-red-500 text-sm mr-2">
                {saveErrorEndometrialPreparation}
              </p>
            )}
            {saveSuccessEndometrialPreparation && (
              <p className="text-green-500 text-sm mr-2">Lưu thành công!</p>
            )}
            ...
            {isEditable && (
              <Button
                onClick={handleSaveEndometrialPreparation}
                disabled={isSavingEndometrialPreparation}
              >
                <Save className="w-4 h-4 mr-2" />{" "}
                {isSavingEndometrialPreparation ? "Đang lưu..." : "Lưu"}
              </Button>
            )}
          </DialogFooter>
        </DialogContent>
